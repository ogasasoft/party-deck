...
- 手札は1人1枚。
- お題文はオリジナル。
- setup画面で今回のお題を事前表示し、同カテゴリ内で再抽選できる。開始後はそのお題IDを使う。
...
- 使用カードはプレイヤー数+2枚。
- 中央カードは2枚。
- setup画面の役職カードは人数+2枚の有効構成に補正する。全0枚などの保存済み不正構成はおすすめ構成へ戻す。
- 占い師、人狼、怪盗の順に夜行動。
- 怪盗交換後の最終役職で勝敗判定。
...
- 人間関係へ踏み込むゲームは除外せず、`intensity: "strong"` と `contentWarnings` を設定する。
- 刺激強めゲームは一覧時点で含まれる話題を表示し、ルールに回答拒否や途中終了の方法を含める。
...
- 刺激強めなら `intensity` と `contentWarnings` が設定され、刺激度フィルタへ表示される。
